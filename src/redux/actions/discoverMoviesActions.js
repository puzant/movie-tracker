...
const fetchMoviesSuccess = createAction(
  GET_MOVIES_ACTIONS.SUCCESS,
  (movies) => ({ movies })
);
const fetchMoviesError = createAction(GET_MOVIES_ACTIONS.ERROR, (error) => ({
  error,
}));
  ...
  return async (dispatch) => {
    try {
      ...
      const results = await dispatch(
        fetchMoviesSuccess(getMoviesResponse.data.results)
      );
      return results;
    } catch (error) {
...
const fetchMoviesGenresSuccess = createAction(GET_MOVIES_GENRES, (genres) => ({
  genres,
}));
...
const fetchMoreMoviesSuccess = createAction(
  GET_MORE_MOVIES_ACTIONS.SUCCESS,
  (movies) => ({ movies })
);
const fetchMoreMoviesError = createAction(
  GET_MORE_MOVIES_ACTIONS.ERROR,
  (error) => ({ error })
);
...
const filter = createAction(
  FILTER_MOVIES_ACTIONS.FILTER_MOVIES,
  (movies, filterType) => ({ movies, filterType })
);
const sort = createAction(
  SORTING_MOVIES_ACTIONS.SORT_MOVIES,
  (movies, sortingType) => ({ movies, sortingType })
);
const filterByGenres = createAction(
  FILTER_MOVIES_ACTIONS.FILTER_BASED_ON_GENRES,
  (genre) => ({ genre })
);
